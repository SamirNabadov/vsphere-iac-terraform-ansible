resource "vsphere_virtual_machine" "web" {
  count            = 3
  name             = "${var.resource_name["web"]}-${1+count.index}"
  ...
  clone {
    ...
    customize {
      linux_options {
        host_name = "${var.resource_name["web"]}-${1+count.index}"
        domain    = "${var.domain}"
      }

      network_interface {
        
        ipv4_address = "${var.subnet}.${91+count.index}"
  ...
  provisioner "remote-exec"  {
    ...
    connection {
        ...
        host    = "${var.subnet}.${91+count.index}"
    }
  }
}